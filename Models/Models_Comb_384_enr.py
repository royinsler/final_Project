...
def get_transforms(feature_extractor):
    ...
    _train_transform = Compose([transforms.ColorJitter(brightness=0.1, contrast=0.1, saturation=0.1, hue=0.1),
                                SafeResize((384, 384)),  # Custom resizing with error handling
                                RandomVerticalFlip(0.5),
                                RandomRotation(10),
                                RandomHorizontalFlip(0.5),
                                ToTensor(),
                                normalize])
    _transforms = Compose([SafeResize((384, 384)),ToTensor(), normalize])
...
plt.title(f'Training and validation Losses for Models 384 res with enriched db')
plt.savefig(f'Loss_along_epochs_all_models_384_enr.jpg')