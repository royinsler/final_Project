...
model_types = {
    ...
    #     'fine-tuned_model_patch': '/mnt/md0/royi/final_Project/Swin_output/checkpoint-13326/model.safetensors'
    ...
    # },
    'Convnext': {
            'model_path': 'facebook/convnext-base-384',
    ...
    # 'BeiT': {
    #     'model_path': 'microsoft/beit-base-patch16-384',
...
for model_name, model_info in model_types.items():
    ...
    transform = Compose([transforms.Resize(384),ToTensor(), normalize])
...
# data
    ...
    plt.title(f'jpg_based Confusion Matrix {model_name} 384')
    plt.savefig(f'jpg_based_confusion_matrix_{model_name} 384.jpg')
    ...
    results.to_csv(f"results_test_scan-based_{model_name}_384.csv", index=False)
    ...
    plt.xlabel('Predicted Label')
    plt.ylabel('True Label')
    plt.title(f'Scan-based Confusion Matrix_{model_name}_384')
    plt.savefig(f'scan_based_confusion_matrix_{model_name}_384.jpg')
    plt.close()
    ...
    # subject based
    df = pd.read_csv(f"results_test_scan-based_{model_name}_384.csv")
    ...
    results.to_csv(f"results_test_case-based_{model_name}_384.csv", index=False)
    ...
    plt.xlabel('Predicted Label')
    plt.ylabel('True Label')
    plt.title(f'Case-based Confusion Matrix {model_name} 384')
    plt.savefig(f'case_based_confusion_matrix_{model_name} 384.jpg')
...
for case_name in case_names:
    ...
    plt.title(case_name+' ROC Curves for Multiple Models Across 384')
    ...
    # plt.tight_layout()
    # plt.show()
    plt.savefig(case_name+'_ROC_384.jpg')
    plt.close()
    plt.clf()